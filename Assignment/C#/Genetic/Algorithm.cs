...
namespace Genetic
{
    class Algorithm<A>
    {
        ...
        public A Run(Func<A> createAividual, Func<A,double> computeFitness, Func<A[],double[],Func<Tuple<A,A>>> selectTwoParents, 
            Func<Tuple<A,A>,Tuple<A,A>> crossover, Func<A, double, A> mutation)
        {
            // initialize the first population
            var initialPopulation = Enumerable.Range(0, populationSize).Select(i => createAividual()).ToArray();
            ...
            for (int generation = 0; generation < numIterations; generation++)
            {
                ...
                {
                    startAex = 1;
                    var populationWithFitness = currentPopulation.Select((individual, index) => new Tuple<A,double>(individual,fitnesses[index]));
            ...
            return currentPopulation.Select((individual, index) => new Tuple<A, double>(individual, finalFitnesses[index])).OrderByDescending(tuple => tuple.Item2).First().Item1;
        }
    }
}
